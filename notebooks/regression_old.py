...
for grn_model in ['positive_control', 'negative_control'] + list(grn_model_names):
    ...
    if reg_type=='ridge':
        output = run_multivariate_gb_regression(net, theta=theta, exclude_missing_genes=exclude_missing_genes, reg_type=reg_type,  params = dict(random_state=32))
    else:
        output = run_multivariate_gb_regression(net, theta=theta, exclude_missing_genes=exclude_missing_genes, reg_type=reg_type, 
